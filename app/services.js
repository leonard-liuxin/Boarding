...
services.factory('$service', ['$http', 'serviceUrl', 'md5', function ($http, serviceUrl, md5) {
    return {
        login: function (info, callback, errCallback) {
            $http({ method: 'POST', cache: false, url: serviceUrl + 'authenticate', data: info, withCredentials: true })
        ...
        getPrivileges: function (info, callback, errCallback) {
            $http({ method: 'POST', cache: false, url: serviceUrl + 'boarding/get-privileges', data: info, withCredentials: true })
                .success(function (data) {
                    if (data && !data.error) {
                        callback(data);
                    } else {
        ...
        search: function (info, callback, errCallback) {
            $http({ method: 'POST', cache: false, url: serviceUrl + 'boarding/search', data: info, withCredentials: true })
                .success(function (data) {
                    if (data && !data.error) {
                        callback(data);
                    } else {
        ...
        changeStatus: function (action, info, callback, errCallback) {
            $http({ method: 'POST', cache: false, url: serviceUrl + 'boarding/' + action, data: info, withCredentials: true })
                .success(function (data) {
                    if (data && !data.error) {
                        callback(data);
                    } else {
